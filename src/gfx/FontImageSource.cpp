...
namespace dagui
{
	...
	ImageDef* FontImageSource::item()
	{
		...
		auto* image = new GlyphImageDef(_face, _face->glyph->glyph_index, _face->glyph->bitmap.width, _face->glyph->bitmap.rows, _face->glyph->advance.x>>6);//, 1, _face->glyph->bitmap.buffer);
		return image;
	}
